...
class CBaseMatrix : public D3DXMATRIX
{
    ...
    void _CAssertSize_() 
    {
        static_assert(sizeof(CBaseMatrix) == sizeof(D3DXMATRIX), "Size is not correct");
    }

public:

    CBaseMatrix() : D3DXMATRIX() {}
    ...
    CBaseMatrix(__in_ecount(1) const CBaseMatrix &m) : D3DXMATRIX(m) { }
    CBaseMatrix(const D3DMATRIX& m) : D3DXMATRIX(m) {}
    CBaseMatrix(const D3DMATRIX*& m) : D3DXMATRIX(*m) {}
...
protected:
    ...
    CBaseMatrix(__in_ecount(16) const float * pf) : D3DXMATRIX(pf) { }
...
public:
    inline static __returnro CBaseMatrix const *ReinterpretBase(
        __in_ecount(1) D3DXMATRIX const *pm
        )
    {
